...
public class InPlaceUserMovement_IOS1 : MonoBehaviour
{
    ...
    void PositionControl()
    {
        ...
        if (Mathf.Abs(arCamRotDiff.x) > Mathf.Abs(arCamRotDiff.y) && arCamRotDiff.x > 0)
        {
            ...
            moveStep = Vector3.Scale(arCamForward * Mathf.Abs(arCamRotDiff.x) * 0.02f * Time.deltaTime, new Vector3(1, 0, 1));
        ...
        if (_animator)
        {
            _animator.SetBool(_animIDSpeed, (Vector3.Distance(Vector3.zero, moveStep) > 0.1f));
        }
    ...
    bool IsTouch()
    {
        ...
        arCamRotDiff = arCamCurrRot - arCamLastRot;
        arCamRotDiff = new Vector3(-arCamRotDiff.x, -arCamRotDiff.z, arCamRotDiff.y);
        if (Mathf.Abs(arCamRotDiff.x) > 30)
            return true;
        return false;
    }
    ...
    void RotationControl()
    {
        ...
        else
        {
            ...
            gripPoint.transform.localEulerAngles += Vector3.Scale(arCamRotDiff, new Vector3(1, 0, 0));
        }
